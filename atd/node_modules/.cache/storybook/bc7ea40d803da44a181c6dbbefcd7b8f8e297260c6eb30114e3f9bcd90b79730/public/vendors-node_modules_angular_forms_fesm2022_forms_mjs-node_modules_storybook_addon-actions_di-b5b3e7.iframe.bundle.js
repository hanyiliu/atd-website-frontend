"use strict";
(self["webpackChunkatd"] = self["webpackChunkatd"] || []).push([["vendors-node_modules_angular_forms_fesm2022_forms_mjs-node_modules_storybook_addon-actions_di-b5b3e7"],{
 ...
 * @publicApi
 */
const VERSION = new _angular_core__WEBPACK_IMPORTED_MODULE_0__.Version('19.2.14');
...
var PARAM_KEY = "actions",
  ...
  CLEAR_ID = `${ADDON_ID}/action-clear`,
  CYCLIC_KEY = "$___storybook.isCyclic";
var config = {
    depth: 10,
...
var findProto = (obj, callback) => {
  ...
  isReactSyntheticEvent = e => !!(typeof e == "object" && e && findProto(e, proto => /^Synthetic(?:Base)?Event$/.test(proto.constructor.name)) && typeof e.persist == "function"),
  serializeArg = a => {
  ...
  generateId = () => typeof crypto == "object" && typeof crypto.getRandomValues == "function" ? (0,uuid__WEBPACK_IMPORTED_MODULE_3__["default"])() : Date.now().toString(36) + Math.random().toString(36).substring(2);
function action(name, options = {}) {
  let actionOptions = {
    ...
    handler = function (...args) {
      if (options.implicit) {
        let storyRenderer = ("__STORYBOOK_PREVIEW__" in _storybook_global__WEBPACK_IMPORTED_MODULE_2__.global ? _storybook_global__WEBPACK_IMPORTED_MODULE_2__.global.__STORYBOOK_PREVIEW__ : void 0)?.storyRenders.find(render => render.phase === "playing" || render.phase === "rendering");
        if (storyRenderer) {
          let deprecated = !globalThis?.FEATURES?.disallowImplicitActionsInRenderV8,
            error = new storybook_internal_preview_errors__WEBPACK_IMPORTED_MODULE_1__.ImplicitActionsDuringRendering({
              phase: storyRenderer.phase,
              name,
              deprecated
            });
          if (deprecated) console.warn(error);else throw error;
        }
      }
      let channel = storybook_internal_preview_api__WEBPACK_IMPORTED_MODULE_0__.addons.getChannel(),
        id = generateId(),
        minDepth = 5,
...
/* harmony export */ });
/* harmony import */ var _chunk_4XZ63LWV_mjs__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./chunk-4XZ63LWV.mjs */ "./node_modules/@storybook/addon-actions/dist/chunk-4XZ63LWV.mjs");
...
var preview_exports = {};
(0,_chunk_4XZ63LWV_mjs__WEBPACK_IMPORTED_MODULE_0__.__export)(preview_exports, {
  argsEnhancers: () => argsEnhancers,
  loaders: () => loaders
});
var isInInitialArgs = (name, initialArgs) => typeof initialArgs[name] > "u" && !(name in initialArgs),
  inferActionsFromArgTypesRegex = context => {
    let {
      initialArgs,
      argTypes,
      id,
    ...
    return Object.entries(argTypes).filter(([name]) => !!argTypesRegex.test(name)).reduce((acc, [name, argType]) => (isInInitialArgs(name, initialArgs) && (acc[name] = (0,_chunk_4XZ63LWV_mjs__WEBPACK_IMPORTED_MODULE_0__.action)(name, {
      implicit: !0,
      id
    })), acc), {});
  },
  addActionsFromArgTypes = context => {
    ...
    return actions2?.disable || !argTypes ? {} : Object.entries(argTypes).filter(([name, argType]) => !!argType.action).reduce((acc, [name, argType]) => (isInInitialArgs(name, initialArgs) && (acc[name] = (0,_chunk_4XZ63LWV_mjs__WEBPACK_IMPORTED_MODULE_0__.action)(typeof argType.action == "string" ? argType.action : name)), acc), {});
  };
var argsEnhancers = [addActionsFromArgTypes, inferActionsFromArgTypesRegex];
var subscribed = !1,
  logActionsWhenMockCalled = context => {
    let {
      parameters: {
        actions: actions2
      }
    } = context;
    if (!actions2?.disable && !subscribed && "__STORYBOOK_TEST_ON_MOCK_CALL__" in _storybook_global__WEBPACK_IMPORTED_MODULE_2__.global && typeof _storybook_global__WEBPACK_IMPORTED_MODULE_2__.global.__STORYBOOK_TEST_ON_MOCK_CALL__ == "function") {
      let onMockCall = _storybook_global__WEBPACK_IMPORTED_MODULE_2__.global.__STORYBOOK_TEST_ON_MOCK_CALL__;
      onMockCall((mock, args) => {
        let name = mock.getMockName();
        name !== "spy" && (!/^next\/.*::/.test(name) || ["next/router::useRouter()", "next/navigation::useRouter()", "next/navigation::redirect", "next/cache::", "next/headers::cookies().set", "next/headers::cookies().delete", "next/headers::headers().set", "next/headers::headers().delete"].some(prefix => name.startsWith(prefix))) && (0,_chunk_4XZ63LWV_mjs__WEBPACK_IMPORTED_MODULE_0__.action)(name)(args);
      }), subscribed = !0;
    }
  },
  loaders = [logActionsWhenMockCalled];
var index_default = () => (0,storybook_internal_preview_api__WEBPACK_IMPORTED_MODULE_1__.definePreview)(preview_exports);
...
//# sourceMappingURL=vendors-node_modules_angular_forms_fesm2022_forms_mjs-node_modules_storybook_addon-actions_di-b5b3e7.iframe.bundle.js.map